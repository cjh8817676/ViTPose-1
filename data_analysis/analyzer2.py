...
def  calculate_segment_cg(pointlist,R_proximal):
    p_proximal, p_distal = pointlist[0],pointlist[1]
    """
    Calculate the center of gravity(cg) of body segment, R_proximal is the ratio from which we want to calculate.
    The original matrix to calculate cg of segment is:
    p_cg = p_proximal+R_proximal*(p_distal-p_proximal)
    where p_cg and others p_ is the list [x,y]
    """
        # Calculate the center of gravity (cg) of the body segment
    p_cg = [p_proximal[0] + R_proximal * (p_distal[0] - p_proximal[0]),
            p_proximal[1] + R_proximal * (p_distal[1] - p_proximal[1])]
    
    return p_cg

def calculate_body_cg(total_seg):
    P = [0.08, 0.49, 0.05, 0.05, 0.16, 0.16]
    assert len(total_seg) == len(P), "肢段數量和質量比例數量不一致"

    body_cg = [0, 0]  # 初始化人體質心座標

    for i in range(len(total_seg)):
        seg_cg = total_seg[i]  # 取得肢段質心座標
        mass_ratio = P[i]  # 取得肢段質量比例

        body_cg[0] += seg_cg[0] * mass_ratio  # X座標乘以質量比例後加總
        body_cg[1] += seg_cg[1] * mass_ratio  # Y座標乘以質量比例後加總
    ...
    def open_file(self):
        ...
        while(self.stream.isOpened()):
            ...
            try:
                ...
                # Calculate head_cg
                head_cg = [(self.l_eye[counter][0] + self.r_eye[counter][0]) / 2, (self.l_eye[counter][1] + self.r_eye[counter][1]) / 2]
                
                # Calculate the center of gravity for other segments
                r_arm_cg = calculate_segment_cg([self.r_shoulder[counter], self.r_wrist[counter]], 0.53)
                l_arm_cg = calculate_segment_cg([self.l_shoulder[counter], self.l_wrist[counter]], 0.53)
                
                shoulder_center = [(self.r_shoulder[counter][0]+self.l_shoulder[counter][0])/2, (self.r_shoulder[counter][1]+self.l_shoulder[counter][1])/2]
                hip_center = [(self.r_hip[counter][0]+self.l_hip[counter][0])/2, (self.r_hip[counter][1]+self.l_hip[counter][1])/2]
                trunk_cg = calculate_segment_cg([shoulder_center, hip_center], 0.495)
                
                r_leg_cg = calculate_segment_cg([self.r_hip[counter], self.r_ankle[counter]], 0.447)
                l_leg_cg = calculate_segment_cg([self.l_hip[counter], self.l_ankle[counter]], 0.447)
                
                total_seg = [head_cg, trunk_cg, r_arm_cg, l_arm_cg, r_leg_cg, l_leg_cg]
